      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2 flex-wrap">
            {/* RSI Filters */}
            {[
              { id: 'oversold', label: '🔴 Oversold <30', desc: 'RSI 20-30' },
              { id: 'extreme', label: '🚨 Extreme <20', desc: 'RSI below 20' },
              { id: 'overbought', label: '🟢 Overbought >70', desc: 'RSI above 70' },
            ].map((filter) => (
              <button
                key={filter.id}
                onClick={() => {
                ...
                className={`px-3 py-2 rounded-xl text-xs whitespace-nowrap transition-all font-medium group relative ${
                  preset === filter.id
              ...
              >
                {filter.label}
                <span className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10 ${
            ...
            {/* Divider */}
            <div className={`w-px h-8 self-center ${darkMode ? 'bg-white/10' : 'bg-gray-300'}`} />
            ...
            {[
              ...
              { id: 'volume', label: '🔥 High Volume', desc: 'High volume/mcap ratio' },
            ].map((filter) => (
              <button
                key={filter.id}
                onClick={() => {
            ...
            {[
              { id: 'above_sma50', label: '📈 Above SMA50', desc: 'Price > 50-day SMA', enhanced: true },
              { id: 'below_bb', label: '⚠️ Below BB', desc: 'Below Bollinger Band', enhanced: true },
              { id: 'volume_spike', label: '🔥 Volume Spike', desc: 'Volume > 1.5x avg', enhanced: true },
              { id: 'has_funding', label: '💰 Has Futures', desc: 'Funding rate available', enhanced: true },
              { id: 'negative_funding', label: '💵 Negative Funding', desc: 'Shorts paying longs', enhanced: true },
            ].map((signal) => (
              <button
                key={signal.id}
                onClick={() => {
                  setPreset(null);
                  toggleSignalFilter(signal.id);
                }}
              ...
              >
                ...
                <span className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-10 ${
                  ...
                  {signal.desc}
                  {signal.enhanced && !useEnhancedAPI && <div className="text-orange-400 mt-1">Enable enhanced data</div>}
                </span>
              </button>
            ))}
          </div>
