      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-8">
        ...
        {watchedTokens.length === 0 ? (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-12 text-center">
            ...
            <button 
              onClick={onBack}
              className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 rounded-xl font-medium hover:opacity-90 transition-opacity"
            >
              Browse Tokens
            </button>
          </div>
        ...
        {watchedTokens.length > 0 && (
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-xl font-bold">
                {(watchedTokens.filter(t => t.rsi !== null).reduce((sum, t) => sum + t.rsi, 0) / 
                  watchedTokens.filter(t => t.rsi !== null).length || 0).toFixed(1)}
              </p>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              ...
              <p className={`text-xl font-bold ${
                (watchedTokens.reduce((sum, t) => sum + (t.change24h || 0), 0) / watchedTokens.length) >= 0 
...
export default function App() {
  ...
  const [currentPage, setCurrentPage] = useState('home'); // 'home', 'methodology', 'terms', 'privacy', 'watchlist'
  ...
  // Show Watchlist page (requires login)
