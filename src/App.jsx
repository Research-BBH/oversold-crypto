...
// Theme Toggle Component - defined early so it can be used in all page components
...
const MethodologyPage = ({ onBack, darkMode, setDarkMode }) => (
  <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-gray-900'}`}>
    ...
    <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
      ...
      <p className={`text-lg mb-12 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>How we calculate RSI and analyze cryptocurrency markets</p>
      
      {/* Feature Cards */}
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>We calculate the 14-day Relative Strength Index (RSI) for the top 1000 cryptocurrencies daily to identify oversold (RSI &lt; 30) and overbought (RSI &gt; 70) conditions.</p>
        </div>
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Data is refreshed every minute from CoinGecko's API, providing you with up-to-date price movements, volume, and market cap information for accurate analysis.</p>
        </div>
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Sign in with Google to save tickers to your personal watchlist. Track your favorite cryptocurrencies and export to CSV format for further analysis.</p>
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-8 mb-8`}>
        ...
        <p className={`mb-6 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>The Relative Strength Index (RSI) is a momentum oscillator developed by J. Welles Wilder in 1978. It measures the speed and magnitude of price movements on a scale from 0 to 100, helping traders identify potential overbought or oversold conditions in the market.</p>
        
        <h3 className="text-orange-400 font-semibold text-lg mb-3">The Formula</h3>
        <div className={`${darkMode ? 'bg-black/30' : 'bg-gray-100'} rounded-xl p-4 mb-4 font-mono text-sm`}>
          <p className={darkMode ? 'text-white' : 'text-gray-900'}>RSI = 100 - (100 / (1 + RS))</p>
          <p className={`mt-2 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Where RS = Average Gain / Average Loss over 14 periods</p>
        </div>
        <p className={`mb-6 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>The calculation involves comparing the average gains to average losses over a 14-day period. This smoothed ratio is then normalized to create a value between 0 and 100.</p>
        
        <h3 className="text-orange-400 font-semibold text-lg mb-3">Step-by-Step Calculation</h3>
        <ol className={`space-y-2 mb-6 list-decimal list-inside ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
        ...
        <div className={`${darkMode ? 'bg-black/30' : 'bg-gray-100'} rounded-xl p-4 mb-6 font-mono text-sm`}>
          <p className={darkMode ? 'text-white' : 'text-gray-900'}>Avg Gain = [(Prev Avg Gain) × 13 + Current Gain] / 14</p>
          <p className={darkMode ? 'text-white' : 'text-gray-900'}>Avg Loss = [(Prev Avg Loss) × 13 + Current Loss] / 14</p>
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-8 mb-8`}>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className={`${darkMode ? 'bg-orange-500/10 border-orange-500/20' : 'bg-orange-50 border-orange-200'} border rounded-xl p-5`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-3 h-3 rounded-full bg-orange-500"></span>
              <h3 className="font-bold text-orange-500">Oversold (RSI &lt; 30)</h3>
            </div>
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Indicates the asset may be undervalued. Selling pressure has been dominant, potentially creating buying opportunities. Consider researching for potential entry points.</p>
          </div>
          <div className={`${darkMode ? 'bg-red-500/10 border-red-500/20' : 'bg-red-50 border-red-200'} border rounded-xl p-5`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-3 h-3 rounded-full bg-red-500"></span>
              <h3 className="font-bold text-red-500">Extreme (RSI &lt; 20)</h3>
            </div>
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Extremely oversold conditions. The asset has experienced significant selling pressure. These conditions often precede sharp rebounds, but always do additional research.</p>
          </div>
          <div className={`${darkMode ? 'bg-gray-500/10 border-gray-500/20' : 'bg-gray-100 border-gray-200'} border rounded-xl p-5`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-3 h-3 rounded-full bg-gray-400"></span>
              <h3 className={`font-bold ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Neutral (RSI 30-70)</h3>
            </div>
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>The asset is in neutral territory with balanced buying and selling pressure. No extreme conditions detected. Watch for trends developing toward either extreme.</p>
          </div>
          <div className={`${darkMode ? 'bg-green-500/10 border-green-500/20' : 'bg-green-50 border-green-200'} border rounded-xl p-5`}>
            <div className="flex items-center gap-2 mb-3">
              <span className="w-3 h-3 rounded-full bg-green-500"></span>
              <h3 className="font-bold text-green-500">Overbought (RSI &gt; 70)</h3>
            </div>
            <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Indicates the asset may be overvalued. Buying pressure has been dominant. Consider taking profits or waiting for a pullback before entering new positions.</p>
          </div>
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-8 mb-8`}>
        ...
        <div className={`space-y-4 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
          <p><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Data Provider:</span> All market data is sourced from CoinGecko's comprehensive cryptocurrency API, providing coverage for the top 1000 tokens by market capitalization.</p>
          <p><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Update Frequency:</span> Data is refreshed automatically every 60 seconds to ensure you have access to near real-time market conditions.</p>
          <p><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>RSI Period:</span> We use the standard 14-period RSI as recommended by J. Welles Wilder, the original creator of the indicator.</p>
        </div>
        <div className={`mt-6 ${darkMode ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-yellow-50 border-yellow-300'} border rounded-xl p-4`}>
          <div className="flex items-start gap-3">
            <span className="text-yellow-500 text-xl">⚠️</span>
            <div>
              <p className="text-yellow-500 font-semibold">Important:</p>
              <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>RSI is just one indicator among many. It should not be used in isolation for trading decisions. Always combine RSI with other technical indicators, fundamental analysis, and proper risk management. Past performance does not guarantee future results.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <footer className={`text-center py-8 border-t ${darkMode ? 'border-white/10' : 'border-gray-200'}`}>
        <p className={`text-sm mb-4 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>Nothing on this site is financial advice. For educational and entertainment purposes only.</p>
...
const TermsPage = ({ onBack, darkMode, setDarkMode }) => (
  <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-gray-900'}`}>
    ...
    <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
      ...
      <p className={`text-lg mb-8 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Last updated: January 2025</p>

      <div className="space-y-6">
        {[
          {title: '1. Acceptance of Terms', content: 'By accessing and using OVERSOLD.crypto ("the Service"), you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use the Service.'},
          {title: '2. Description of Service', content: 'OVERSOLD.crypto provides cryptocurrency market data and RSI (Relative Strength Index) analysis tools for educational and informational purposes.'},
          {title: '3. Not Financial Advice', content: 'The information provided on this Service is for educational and entertainment purposes only and should NOT be construed as financial advice. We are not registered financial advisors. RSI and other indicators are tools, not guarantees of performance. Cryptocurrency investments are highly volatile and risky.', isWarning: true},
          {title: '4. User Accounts', content: 'When you create an account using Google Sign-In, you are responsible for maintaining the security of your account and agree to provide accurate information.'},
          {title: '5. Intellectual Property', content: 'All content, design, and functionality of the Service are the property of OVERSOLD.crypto and are protected by copyright and other intellectual property laws.'},
          {title: '6. Data Accuracy', content: 'While we strive to provide accurate data, we cannot guarantee the accuracy, completeness, or timeliness of information. Data is sourced from third-party providers and may contain errors or delays.'},
          {title: '7. Limitation of Liability', content: 'To the maximum extent permitted by law, OVERSOLD.crypto shall not be liable for any indirect, incidental, special, consequential, or punitive damages resulting from your use of the Service.'},
          ...
          <div key={i} className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
            <h2 className="text-xl font-bold mb-4">{section.title}</h2>
            {section.isWarning && <div className={`${darkMode ? 'bg-red-500/10 border-red-500/30' : 'bg-red-50 border-red-200'} border rounded-xl p-4 mb-4`}><p className="text-red-500 font-semibold">⚠️ Important Disclaimer</p></div>}
            <p className={darkMode ? 'text-gray-300' : 'text-gray-700'}>{section.content}</p>
          </div>
        ))}
      </div>

      <footer className={`text-center py-8 mt-8 border-t ${darkMode ? 'border-white/10' : 'border-gray-200'}`}>
        <div className="flex items-center justify-center gap-6 text-sm">
    ...
    <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="space-y-6">
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <p className={`mb-4 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>We collect minimal information to provide our services:</p>
          <h3 className="text-orange-400 font-semibold mb-2">Account Information (if you sign in)</h3>
          <ul className={`space-y-1 list-disc list-inside mb-4 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            <li>Name, email address, profile picture (from Google)</li>
          </ul>
          <h3 className="text-orange-400 font-semibold mb-2">Usage Data</h3>
          <ul className={`space-y-1 list-disc list-inside ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            <li>Watchlist preferences and theme preferences (stored locally)</li>
          </ul>
        </div>
        ...
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <div className={`${darkMode ? 'bg-green-500/10 border-green-500/30' : 'bg-green-50 border-green-200'} border rounded-xl p-4 mb-4`}>
            <p className="text-green-500 font-semibold">✓ Privacy-Focused Design</p>
          </div>
          <ul className={`space-y-2 list-disc list-inside ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            <li><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Local Storage:</span> Your watchlist and preferences are stored locally</li>
            <li><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>No Tracking:</span> We do not use cookies for tracking or advertising</li>
            <li><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>No Selling:</span> We never sell your personal information</li>
          </ul>
        </div>
        ...
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <ul className={`space-y-2 list-disc list-inside ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            <li><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Google Sign-In:</span> For authentication</li>
            <li><span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>CoinGecko API:</span> For cryptocurrency market data</li>
          </ul>
        </div>
        ...
        <div className={`${darkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200'} border rounded-2xl p-6`}>
          ...
          <p className={darkMode ? 'text-gray-300' : 'text-gray-700'}>If you have questions about this Privacy Policy, please contact us through our website.</p>
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-gray-900'} selection:bg-orange-500/30`}>
